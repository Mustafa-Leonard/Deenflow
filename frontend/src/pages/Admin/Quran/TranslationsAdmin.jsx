...
export default function TranslationsAdmin() {
    ...
    const fetchSources = async () => {
        try {
            const resp = await api.get('/quran/admin/translations/')
    ...
    const toggleStatus = async (id, currentStatus) => {
        try {
            await api.patch(`/quran/admin/translations/${id}/`, { is_active: !currentStatus })
            fetchSources()
        } catch (e) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            <div>
                <h1 className="text-3xl font-display font-bold text-slate-900 dark:text-white mb-2 tracking-tight">Translation Sources</h1>
                <p className="text-slate-500 dark:text-slate-400 font-medium">Enable or disable various translations for members.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {sources.map(s => (
                    <Card key={s.id} className="p-6 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl group">
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{s.author} • {s.language}</p>
                        <div className="pt-4 border-t border-slate-50 dark:border-slate-800 flex items-center gap-2">
                            <div className="text-[10px] uppercase font-bold text-slate-400 tracking-widest">ID: {s.id}</div>
                        </div>
                    </Card>
                ))}
                ...
                <Card className="p-8 border-2 border-dashed border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50 flex flex-col items-center justify-center gap-4 group cursor-pointer hover:border-brand-500 transition-all rounded-3xl">
                    ...
                    <div className="text-sm font-bold text-slate-400 group-hover:text-brand-600">Add Translation Source</div>
                </Card>
            </div>
        </div>
    )
